...
university_tier = st.selectbox("University Tier", [1,2,3])
years_experience = st.number_input("Years of Experience",0, 40)
employment_gap_months = st.number_input("Employment Gap (Months)",0, 120) 
technical_skill_score = st.slider("Technical Skill Score",0, 100)
communication_score = st.slider("Communication Score",0, 100)
aptitude_test_score = st.slider("Aptitude Test Score",0, 100)
coding_test_score = st.slider("Coding Test Score",0, 100)
project_count = st.number_input("Project Count",0, 50)
github_activity_score = st.slider("GitHub Activity Score", 0, 100)
certifications_count = st.number_input("Certifications",0, 20)
expected_salary = st.number_input("Expected Salary")
ai_resume_score = st.slider("Ai Resume Score",0, 100)
ai_bias_score = st.slider("Ai Bias Score",0.0, 1.0)
